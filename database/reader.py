...
class QueueBasedImageReader(object):
    ...
    def get_batch(self, batch_size, crop_size, random_mirror, random_blur, random_rotate,
        ...
        if random_rotate:
            ...
            rd_rotatoin = tf.random.uniform([], -10.0, 10.0)
            angle = rd_rotatoin / 180.0 * math.pi

            img = tfa.image.rotate(img, angle, 'BILINEAR')
            label -= 255
            label = tfa.image.rotate(label, angle, 'NEAREST')
            label += 255

        # Randomly crops the images and labels.
        img, label = helper.random_crop_and_pad_image_and_labels(img, label, crop_size, crop_size)

        image_batch, label_batch = tf.compat.v1.train.batch([img, label],
...
class ImageReader(object):
    ...
    def get_batch_iterator(self, batch_size, crop_size, random_mirror, random_blur, random_rotate,
        ...
        images = tf.convert_to_tensor(value=self.image_list, dtype=tf.string)
        labels = tf.convert_to_tensor(value=self.label_list, dtype=tf.string)
