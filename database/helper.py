...
def rotate_image_tensor(image, angle, mode='black'):
    ...
    coord2_vec = tf.reshape(tf.transpose(a=coord2_vec_unordered, perm=[1, 0]), [-1])

    # center coordinates since rotation center is supposed to be in the image center
    coord1_vec_centered = coord1_vec - tf.cast(image_center[0], dtype=tf.int32)
    coord2_vec_centered = coord2_vec - tf.cast(image_center[1], dtype=tf.int32)
    ...
    if mode == 'repeat':
        ...
        coord_old1_clipped = tf.boolean_mask(tensor=coord1_old_nn, mask=tf.logical_not(outside_ind))
        coord_old2_clipped = tf.boolean_mask(tensor=coord2_old_nn, mask=tf.logical_not(outside_ind))

        coord1_vec = tf.boolean_mask(tensor=coord1_vec, mask=tf.logical_not(outside_ind))
        coord2_vec = tf.boolean_mask(tensor=coord2_vec, mask=tf.logical_not(outside_ind))

    coord_old_clipped = tf.cast(tf.transpose(a=tf.stack([coord_old1_clipped, coord_old2_clipped]), perm=[1, 0]), tf.int32)

    # Coordinates of the new image
    coord_new = tf.transpose(a=tf.cast(tf.stack([coord1_vec, coord2_vec]), tf.int32), perm=[1, 0])
    ...
    for image_channel in image_channel_list:
        ...
        image_rotated_channel_list.append(tf.compat.v1.sparse_to_dense(coord_new, [s[0], s[1]], image_chan_new_values,
                                                             background_color, validate_indices=False))

    image_rotated = tf.transpose(a=tf.stack(image_rotated_channel_list), perm=[1, 2, 0])
...
def image_scaling(img, label, scale_rate):
    if scale_rate is None:
        scale = tf.random.uniform([1], minval=0.5, maxval=2.0, seed=None)
    else:
        scale = tf.random.uniform([1], minval=scale_rate[0], maxval=scale_rate[1], seed=None)
    h_new = tf.cast(tf.multiply(tf.cast(tf.shape(input=img)[0], tf.float32), scale), dtype=tf.int32)
    w_new = tf.cast(tf.multiply(tf.cast(tf.shape(input=img)[1], tf.float32), scale), dtype=tf.int32)
...
def image_mirroring(img, label):
    ...
    img = tf.cond(pred=mirror_cond,
                  true_fn=lambda: tf.reverse(img, [1]),
                  false_fn=lambda: img)
    label = tf.cond(pred=mirror_cond,
                    true_fn=lambda: tf.reverse(label, [1]),
...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    offset_height = tf.cond(pred=tf.less(image_shape[0], crop_h),
                            true_fn=lambda: (crop_h - image_shape[0])//2,
                            false_fn=lambda: tf.constant(0))
    offset_width = tf.cond(pred=tf.less(image_shape[1], crop_w),
                           true_fn=lambda: (crop_w - image_shape[1])//2,
                           false_fn=lambda: tf.constant(0))
    ...
    combined_crop = tf.image.random_crop(combined_pad, [crop_h, crop_w, last_image_dim+1])
