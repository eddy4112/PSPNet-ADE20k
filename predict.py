...
def predict(i_ckpt):
    ...
    images_pl = [tf.compat.v1.placeholder(tf.float32, [None, crop_size, crop_size, 3])]
    ...
    init = [tf.compat.v1.global_variables_initializer(), tf.compat.v1.local_variables_initializer()] + reader_init
    sess.run(init)

    loader = tf.compat.v1.train.Saver(max_to_keep=0)
