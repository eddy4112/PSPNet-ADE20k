...
def resize_images(list_images, output_size, method='bilinear', train_conv2dt=False, stride_conv2dt=8):
    ...
    for i in range(len(list_images)):
        with tf.device('/gpu:%d' % i):
            if method == 'nn':
                _x = tf.image.resize(list_images[i], output_size, method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            elif method == 'area':
                _x = tf.image.resize(list_images[i], output_size, method=tf.image.ResizeMethod.AREA)
            elif method == 'cubic':
                _x = tf.image.resize(list_images[i], output_size, method=tf.image.ResizeMethod.BICUBIC)
            else:
                # output always tf.float32.
                _x = tf.image.resize(list_images[i], output_size, method=tf.image.ResizeMethod.BILINEAR)
...
def max_pool(list_input, ksize, stride):
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            _x = tf.nn.max_pool2d(input=list_input[i],
...
def global_avg_pool(list_input):
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            _x = tf.reduce_mean(input_tensor=list_input[i], axis=[1, 2], keepdims=True)
            list_output.append(_x)
    return list_output
...
def conv2d_same(list_input, out_channels, kernel_size, stride,
    ...
    if initializer == 'he':
        if he_init_std is None:
            ...
            std = he_init_std
        initializer = tf.compat.v1.random_normal_initializer(stddev=std)
    else:
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            with tf.compat.v1.variable_scope(scope, reuse=(i>0)):
                ...
                if stride == 1:
                    if rate > 1:
                        ...
                        _x = tf.nn.conv2d(input=list_input[i], filters=kernel, strides=stride_arr(stride), padding='SAME')
                        list_output.append(_x)
                else:
                    ...
                    inputs = tf.pad(tensor=list_input[i],
                                    paddings=[[0, 0], [pad_beg, pad_end], [pad_beg, pad_end], [0, 0]])
                    if rate > 1:
                        _x = tf.nn.atrous_conv2d(inputs, kernel, rate, 'SAME')
                        list_output.append(_x)
                    else:
                        _x = tf.nn.conv2d(input=inputs, filters=kernel, strides=stride_arr(stride), padding='VALID')
                        list_output.append(_x)
    return list_output
...
def batch_norm(list_input, stats_mode,
    ...
    def get_bn_variables(n_out, use_scale, use_bias):
        if use_bias:
            beta = tf.compat.v1.get_variable('beta', [n_out],
                                   initializer=tf.compat.v1.constant_initializer(), trainable=True, dtype=float_type)
        else:
            beta = tf.zeros([n_out], name='beta')
        if use_scale:
            gamma = tf.compat.v1.get_variable('gamma', [n_out],
                                    initializer=tf.compat.v1.constant_initializer(1.0), trainable=True, dtype=float_type)
        else:
            gamma = tf.ones([n_out], name='gamma')
        # x * gamma + beta

        moving_mean = tf.compat.v1.get_variable('moving_mean', [n_out],
                                      initializer=tf.compat.v1.constant_initializer(), trainable=False, dtype=float_type)
        moving_var = tf.compat.v1.get_variable('moving_variance', [n_out],
                                     initializer=tf.compat.v1.constant_initializer(1), trainable=False, dtype=float_type)
    ...
    def update_bn_ema(xn, batch_mean, batch_var, moving_mean, moving_var, decay):
        ...
        update_op2 = moving_averages.assign_moving_average(
            ...
            name='var_ema_op')
        slim.add_model_variable(moving_mean)
        slim.add_model_variable(moving_var)
    ...
    if len(list_input) == 1:
        ...
        with tf.compat.v1.variable_scope(scope):
            beta, gamma, moving_mean, moving_var = get_bn_variables(n_out, use_gamma, use_beta)
            xn, batch_mean, batch_var = tf.compat.v1.nn.fused_batch_norm(
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            batch_mean = tf.reduce_mean(input_tensor=list_input[i], axis=[0, 1, 2])
            batch_square_mean = tf.reduce_mean(input_tensor=tf.square(list_input[i]), axis=[0, 1, 2])
    ...
    with tf.device('/gpu:0'):  # if your GPUs have NVLinks and you've install NCCL2, you can change `/cpu:0` to `/gpu:0`
        shape = tf.shape(input=list_input[0])
        num = shape[0] * shape[1] * shape[2] * len(list_input)
        batch_mean = tf.reduce_mean(input_tensor=means, axis=0)
        batch_var = tf.reduce_mean(input_tensor=square_means, axis=0) - tf.square(batch_mean)
...
def bottleneck_residual(list_input, out_channels, stride, rate=1, initializer='he',
    ...
    # ======================== Main operations =============================
    orig_x = list_input
    with tf.compat.v1.variable_scope('bottleneck_v1/conv1'):
        """1x1, 64->64"""
    ...
    with tf.compat.v1.variable_scope('bottleneck_v1/conv2'):
        """3x3, 64->64"""
        x = conv2d_same(x, out_channels // 4, 3, stride, rate, initializer)
        x = batch_norm(x, bn_stat_mode, bn_use_gamma, bn_use_beta, bn_epsilon, bn_ema)
        x = relu(x)

    with tf.compat.v1.variable_scope('bottleneck_v1/conv3'):
        """1x1, 64->256"""
    ...
    if in_channels != out_channels:
        with tf.compat.v1.variable_scope('bottleneck_v1/shortcut'):
...
def bottleneck_residual_v2(list_input, out_channels, stride, rate=1, initializer='he',
    ...
    # ======================== Main operations =============================
    with tf.compat.v1.variable_scope('bottleneck_v2'):
...
def fully_connected(list_input, out_channels, initializer='he'):
    ...
    for i in range(len(list_x)):
        with tf.device('/gpu:%d' % i):
            with tf.compat.v1.variable_scope('biases', reuse=(i > 0)):
                b = tf.compat.v1.get_variable('', [out_channels], initializer=tf.compat.v1.constant_initializer(0.01))
...
def conv_bias_relu(list_input, out_channels, kernel_size, stride, relu=True,
    ...
    for i in range(len(list_x)):
        with tf.device('/gpu:%d' % i):
            with tf.compat.v1.variable_scope(bias_scope, reuse=(i > 0)):
                b = tf.compat.v1.get_variable('', [out_channels], initializer=tf.compat.v1.constant_initializer(0.01))
                output = tf.nn.bias_add(list_x[i], b)
...
def conv2d_transpose(name, list_input, out_channels=None,
    ...
    tf.compat.v1.add_to_collection('init_conv2dt_weights', init_conv2dt_weights)

    init = tf.compat.v1.constant_initializer(value=weights,
                                   dtype=tf.float32)

    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            with tf.compat.v1.variable_scope(name, reuse=(i>0)):
...
def dropout(list_input, keep_prob):
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            output = tf.nn.dropout(list_input[i], rate=1 - (keep_prob))
            list_output.append(output)
    return list_output
...
def avg_pool(list_input, ksize, strides, padding):
    ...
    for i in range(len(list_input)):
        with tf.device('/gpu:%d' % i):
            output = tf.nn.avg_pool2d(input=list_input[i], ksize=ksize, strides=strides, padding=padding)
            list_output.append(output)
    return list_output
