...
class PSPNetMG(object):
    def __init__(self, num_classes, resnet='resnet_v1_101', gpu_num=1,
        ...
        self.wd_rate_ph = tf.compat.v1.placeholder(utils.float_type, shape=())
        self.wd_rate2_ph = tf.compat.v1.placeholder(utils.float_type, shape=())
        self.lrn_rate_ph = tf.compat.v1.placeholder(utils.float_type, shape=())
        #
        if optimizer == 'sgd':
            self.optimizer = tf.compat.v1.train.GradientDescentOptimizer(self.lrn_rate_ph)
        elif optimizer == 'rmsp':
            self.optimizer = tf.compat.v1.train.RMSPropOptimizer(self.lrn_rate_ph)
        elif optimizer == 'adam':
            self.optimizer = tf.compat.v1.train.AdamOptimizer(self.lrn_rate_ph, self.momentum)
        elif optimizer == 'mom':
            self.optimizer = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph, self.momentum)
        else:
    ...
    def build_train_ops(self, images, labels, weights=None):
        ...
        # <construct network>
        with tf.compat.v1.variable_scope(self.resnet, reuse=self.reuse):
        ...
        for i in range(len(labels)):
            with tf.device('/gpu:%d' % i):
                ...
                indice = tf.compat.v1.where(tf.logical_and(tf.less(label, self.num_classes), tf.greater_equal(label, 0)))
                label = tf.cast(tf.gather(label, indice), tf.int32)
                num_valide_pixel += tf.shape(input=label)[0]
                ...
                precision_op, update_op = slim.metrics.streaming_mean_iou(prediction, label, num_classes=self.num_classes)
        ...
        loss = tf.truediv(tf.reduce_sum(input_tensor=losses), num_valide_pixel, name='loss')
        aux_loss = tf.truediv(tf.reduce_sum(input_tensor=aux_losses), num_valide_pixel, name='aux_loss')
    ...
    def build_inference_ops(self, one_image, crop_size):
        one_image = tf.convert_to_tensor(value=one_image)
        shape_image = tf.shape(input=one_image)
    ...
    def _normal_loss(self, logits, labels, weights=None):
        ...
        return tf.reduce_sum(input_tensor=xent)

    def _apply_gradients_from_cost(self, cost):
        return self.optimizer.minimize(cost, self.global_step)
    ...
    def _compute_gradients_different_lr(self, cost):
        ...
        if self.optimizer == 'sgd':
            opt_existing = tf.compat.v1.train.GradientDescentOptimizer(self.lrn_rate_ph)
            opt_new_norm = tf.compat.v1.train.GradientDescentOptimizer(self.lrn_rate_ph * 10)
            opt_new_bias = tf.compat.v1.train.GradientDescentOptimizer(self.lrn_rate_ph * 20)
        elif self.optimizer == 'mom':
            opt_existing = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph, self.momentum)
            opt_new_norm = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph * 10, self.momentum)
            opt_new_bias = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph * 20, self.momentum)
        elif self.optimizer == 'rmsp':
            opt_existing = tf.compat.v1.train.RMSPropOptimizer(self.lrn_rate_ph)
            opt_new_norm = tf.compat.v1.train.RMSPropOptimizer(self.lrn_rate_ph)
            opt_new_bias = tf.compat.v1.train.RMSPropOptimizer(self.lrn_rate_ph)
        elif self.optimizer == 'adam':
            opt_existing = tf.compat.v1.train.AdamOptimizer(self.lrn_rate_ph, self.momentum)
            opt_new_norm = tf.compat.v1.train.AdamOptimizer(self.lrn_rate_ph, self.momentum)
            opt_new_bias = tf.compat.v1.train.AdamOptimizer(self.lrn_rate_ph, self.momentum)
        else:
            opt_existing = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph, self.momentum)
            opt_new_norm = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph * 10, self.momentum)
            opt_new_bias = tf.compat.v1.train.MomentumOptimizer(self.lrn_rate_ph * 20, self.momentum)
        ...
        grads = tf.gradients(ys=cost, xs=existing_weights + new_normal_weights + new_bias_weights)
    ...
    def _decay(self, mode):
        ...
        if mode == 0:
            ...
            reader = tf.compat.v1.train.NewCheckpointReader(self.fine_tune_filename)
            print(self.fine_tune_filename)
            losses_existing_layers = 0.0
            losses_new_layers = 0.0
            for v in tf.compat.v1.trainable_variables():
                if 'weights' in v.name:
            ...
            for v in tf.compat.v1.trainable_variables():
                if 'weights' in v.name:
                    ...
                    norm = tf.reduce_sum(input_tensor=tf.multiply(pre_trained_weights, pre_trained_weights),
                                         axis=[0, 1, 2], keepdims=True)
                    cos = tf.reduce_sum(input_tensor=tf.multiply(v, pre_trained_weights), axis=[0, 1, 2], keepdims=True)
            ...
            for v in tf.compat.v1.trainable_variables():
                if 'weights' in v.name:
                    ...
                    # equivalent to this:
                    cos = tf.reduce_sum(input_tensor=tf.multiply(v, pre_trained_weights), axis=[0, 1, 2])
                    cos_square = tf.square(cos)
                    norm = tf.reduce_sum(input_tensor=tf.multiply(pre_trained_weights, pre_trained_weights), axis=[0, 1, 2])
                    losses_existing_layers += tf.nn.l2_loss(v) - 0.5 * tf.reduce_sum(input_tensor=tf.truediv(cos_square, norm))
            ...
            return tf.convert_to_tensor(value=0.0)
...
def pspnet_with_list(images, output_num_classes, training=True,
    ...
    if three_convs_beginning:
        ...
        with tf.compat.v1.variable_scope('conv1_3'):
    ...
    for block_index in range(len(num_residual_units)):
        for unit_index in range(num_residual_units[block_index]):
            with tf.compat.v1.variable_scope('block%d/unit_%d' % (block_index + 1, unit_index + 1)):
                stride = 1
            ...
            if block_index + 1 == 3 and unit_index + 1 == num_residual_units[-2] and has_aux_loss:
                with tf.compat.v1.variable_scope('block%d/unit_%d' % (block_index + 1, unit_index + 2)):
                    # new layers
                ...
                with tf.compat.v1.variable_scope('aux_up_sample'):
            ...
            if block_index + 1 == 4 and unit_index + 1 == num_residual_units[-1]:
                # 4 pooling layers.
                to_concat = [x]
                with tf.compat.v1.variable_scope('psp'):
                    for pool_index in range(len(pool_size)):
                        with tf.compat.v1.variable_scope('pool%d' % pool_rates[pool_index]):
    ...
    with tf.compat.v1.variable_scope('up_sample'):
