...
def train_and_eval():
    ...
    for _ in range(gpu_num()):
        eval_image_pl.append(tf.compat.v1.placeholder(tf.float32, [None, crop_size, crop_size, 3]))
    ...
    init = [tf.compat.v1.global_variables_initializer(), tf.compat.v1.local_variables_initializer()] + train_reader_inits
    sess.run(init)

    # < load pre-trained model>
    import_variables = tf.compat.v1.trainable_variables()
    if FLAGS.fine_tune_filename is not None:
        ...
        loader = tf.compat.v1.train.Saver(var_list=fine_tune_variables, allow_empty=True)
       # step = 0
       #if FLAGS.save_first_iteration == 0:
       #     loader.save(sess, snapshot_dir + '/model.ckpt', global_step=step)
        # 143 ~ 146 I wrote it!
    ...
    while step < max_iter + 1:
        ...
        if step % 100 == 0 or step == max_iter:
            ...
            confusion_matrix = np.zeros((eval_image_reader.num_classes, eval_image_reader.num_classes),
                                        dtype=np.int64)
            sess.run([tf.compat.v1.local_variables_initializer()] + eval_reader_inits)
